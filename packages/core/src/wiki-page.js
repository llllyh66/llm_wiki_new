...
export function findOverlappingWikiPageSections(content, headings) {
  ...
  for (let leftIndex = 0; leftIndex < selected.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < selected.length; rightIndex += 1) {
      ...
      if (descendant.start < ancestor.end) overlaps.push({ ancestor: ancestor.heading, descendant: descendant.heading })
    }
  }
  return overlaps
}
...
export function prepareWikiPageContent(patch, existingContent = "", date = new Date().toISOString().slice(0, 10)) {
  ...
  const lines = [
    ...
    ...(standard.schema_classification_kinds?.length > 0 ? [`schema_classification_kinds: ${yamlArray(standard.schema_classification_kinds)}`] : []),
    ...(standard.schema_domain_keys?.length > 0 ? [`schema_domain_keys: ${yamlArray(standard.schema_domain_keys)}`] : []),
    ...(standard.schema_domain_names?.length > 0 ? [`schema_domain_names: ${yamlArray(standard.schema_domain_names)}`] : []),
    ...(standard.schema_abe_keys?.length > 0 ? [`schema_abe_keys: ${yamlArray(standard.schema_abe_keys)}`] : []),
    ...(standard.schema_abe_names?.length > 0 ? [`schema_abe_names: ${yamlArray(standard.schema_abe_names)}`] : []),
    ...(standard.schema_be_keys?.length > 0 ? [`schema_be_keys: ${yamlArray(standard.schema_be_keys)}`] : []),
    ...(standard.schema_be_names?.length > 0 ? [`schema_be_names: ${yamlArray(standard.schema_be_names)}`] : []),
    ...(standard.schema_classification_paths?.length > 0 ? [`schema_classification_paths: ${yamlArray(standard.schema_classification_paths)}`] : []),
    ...preserved,
    "---",
    "",
    body,
