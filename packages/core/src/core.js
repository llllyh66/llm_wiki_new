  ...
  async #buildBatchResponse({ workspace, record, batch, workerId, expiresAt, lease, requestedBatchChars }) {
    ...
    const response = {
      ...
      analysis_preflight: {
        ...
        grounding_quality: "Exact quote copying is not required. Concise paraphrases are valid when meaningful terms are supported, every identifier/number/date/unit occurs in the selected evidence, and polarity is preserved. Relations additionally require supported endpoints, direction, and predicate.",
        grounding_repair: "On INVALID_ANALYSIS, edit only each grounding_diagnostics path and field, preserve every non-failing candidate and evidence index, keep the same worker and lease, and retry the changed payload with a new idempotency key. Never rebuild the whole envelope merely to make it more verbatim.",
