...
test("get_batch repairs an 81K single-line legacy batch in place and preserves its worker lease", async (t) => {
  ...
  assert.equal(repaired.batch_limits.payload_bytes <= 64 * 1024, true)
  assert.equal(repaired.batch_limits.agent_payload_ceiling_bytes, 64 * 1024)
  assert.equal(repaired.batch_limits.safely_repartitioned, true)
  assert.equal(Buffer.byteLength(JSON.stringify(repaired, null, 2)) < 128 * 1024, true)
